...
if method == "merge":
    ...
    csv_name = f"./score/scifact_score_merge_linear_{alpha_lower}_{alpha_upper}.csv"
elif method == "lora":
    if not args.lora_name:
        parser.error("--lora_name is required when method is 'lora'.")
    lora_name = args.lora_name
    csv_name = f"./score/scifact_score_lora_{lora_name}.csv"
else:
    csv_name = "./score/scifact_score_source.csv"
...
with open(OUTPUT_DIR + "/scifact_queries.jsonl", "w") as f:
    for _,row in df_query.iterrows():
        q = row["text"]
        q = ' '.join(q.split())
        f.write(f'{{"_id": "{str(row["query_id"])}", "text": "{q}"}}\n')

with open(OUTPUT_DIR + "/scifact_corpus.jsonl", "w") as f:
    for _,row in df_doc.iterrows():
        s = row["doc"]
        s = re.sub(r'\\', r'\\\\', s)
        s = re.sub('"', '\\"', s)
        f.write(f'{{"_id": "{str(row["doc_id"])}",  "title": "", "text": "{s}"}}\n')

with open(OUTPUT_DIR + "/scifact_qrels.tsv", "w") as f:
...
query_path = OUTPUT_DIR + "/scifact_queries.jsonl"
qrels_path = OUTPUT_DIR + "/scifact_qrels.tsv"
